...
$("#openbutton").bind("click", function(event, ui) {
    ...
    if (isValidChannel(channelId)) {
      appendTextAndScroll("#output", "Channel already open: (" + channelId + ").");
      return;
    }
...
function iccOpenChannel(aidhexstr) {
  ...
  req.onsuccess = function() {
    ...
    if (!isValidChannel(this.result)) {
      appendTextAndScroll("#output", "INFO: Channel ID invalid. All channels in use.");
    } else {
      channelId = this.result;
    }
  }
  req.onerror = function() {
    appendTextAndScroll("#output", "ERROR: Open Failure! Return data: (" + JSON.stringify(this.error) + ")");
  }
}
...
function iccExchangeAPDU(channel, apdu) {
  ...
  req.onerror = function() {
    appendTextAndScroll("#output", "ERROR: APDU Exchange Failure! Return data: (" + JSON.stringify(this.result) + ")");
  }
}
...
function iccCloseChannel(channel) {
  ...
  req.onerror = function() {
    appendTextAndScroll("#output", "ERROR: Close Failure! Return data: (" + JSON.stringify(this.error) + ")");
  }
}
